...
for IMAGE_REF in ${CLEANUP_IMAGES}; do
    ...
    # But check for / delete the .sig tag in the SIGNATURE repo.
    if ! sig_exists "${SIG_REPO}" "${DIGEST}"; then
        ok "no cosign signature present in ${SIG_REPO} - nothing to do"
        continue
    fi
    log "signature tag present: ${SIG_REPO}:${DIGEST/:/-}.sig"

    if [ "${DRY_RUN}" = "true" ]; then
        log "[DRY-RUN] would delete ${SIG_REPO}:${DIGEST/:/-}.sig"
        continue
    fi
    ...
    if sig_exists "${SIG_REPO}" "${DIGEST}"; then
        ...
        if hub_login; then
            HTTP_CODE="$(hub_delete_tag "${SIG_REPO}" "${SIG_TAG}")"
            log "DELETE /v2/repositories/${SIG_REPO}/tags/${SIG_TAG}/ -> HTTP ${HTTP_CODE}"
        else
