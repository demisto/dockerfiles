...
SEPERATE_PR="/pcap-http-extractor/|/office-utils/|/snowflake/|/faker3/|/ml/|/taxii-server/|/sane-pdf-reports/|/sklearn/|/xgboost/"
...
if [[ -z "$1" ]] || [[ "$1" == -* ]]; then    
    ...
    echo "Will further limit to 25 updates per base image to avoid huge PRs."
    echo ""
...
for image in `echo $BASE_IMAGES` ; do
    ...
    grep -E "FROM\\s+${full_image}:" docker/*/Dockerfile | grep -v "${latest_tag}" | awk -F: '{print $1}' | grep -v -E "$SEPERATE_PR" | grep -v -f $temp_dev | head -$MAX_IMAGES | xargs sed -i '' -e "s#${full_image}:.*#${full_image}:${latest_tag}#"
done
