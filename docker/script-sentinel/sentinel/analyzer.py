...
from .models import AnalysisResult, Finding, Verdict, YaraContribution
...
from .yara_engine import YaraEngine, YaraRuleError, yara_match_to_finding
...
class ScriptAnalyzer:
    ...
    def __init__(self, patterns_dir: Optional[str | Path] = None, strict_mode: bool = False):
        """
        Initializes the script analyzer.

        Args:
            patterns_dir: Directory containing pattern files (optional).
                         If not provided, auto-detects from sentinel/patterns.
            strict_mode: If True, raise YaraRuleError on first invalid Yara rule.
                        If False (default), skip invalid rules with warning log.
        """
        ...
        self.yara_loaded = False

        try:
            self.yara_engine = YaraEngine(rules_dir, strict_mode=self.strict_mode)
            compiled_rules, stats = self.yara_engine.load_rules()
            self.yara_loaded = compiled_rules is not None
            if self.yara_loaded:
                logger.info(f"Yara engine loaded {stats.loaded} rules")
            else:
                logger.warning("Yara engine initialized but no rules loaded")
        except YaraRuleError:
            # Re-raise YaraRuleError for strict mode to fail fast
            raise
        except Exception as e:
            logger.warning(f"Yara engine initialization failed: {e}")
    ...
    def analyze(
        ...
        try:
            ...
            # Step 2.6: Run Yara rule matching
            yara_findings = []
            yara_duration = 0.0

            if self.yara_engine and self.yara_loaded:
                logger.info("Running Yara rule matching")
                yara_start_time = time.time()
                yara_matches = self.yara_engine.match(script_content)
                yara_findings = [yara_match_to_finding(m) for m in yara_matches]
                yara_duration = time.time() - yara_start_time
                logger.info(f"Yara matching found {len(yara_findings)} findings in {yara_duration:.3f}s")

            # Combine Yara findings with heuristic findings
            heuristic_findings.extend(yara_findings)

            # Step 2.7: Aggregate and deduplicate findings from all engines
            # This prevents duplicate detections from inflating risk scores
            pre_dedup_count = len(heuristic_findings)
            logger.info("Aggregating findings from all detection engines")
            aggregation_start_time = time.time()
            heuristic_findings = self.finding_aggregator.aggregate(heuristic_findings)
            aggregation_duration = time.time() - aggregation_start_time
            post_dedup_count = len(heuristic_findings)
            if pre_dedup_count != post_dedup_count:
                logger.info(f"Deduplication: {pre_dedup_count} -> {post_dedup_count} findings "
                           f"({pre_dedup_count - post_dedup_count} duplicates removed)")
            ...
            verdict, confidence, score_breakdown = calculate_verdict(
                findings=all_findings,
                llm_available=llm_available,
                obfuscation_detected=obfuscation_detected,
                paranoia_level=paranoia_level,
                script_content=script_content,
                ast=None if fallback_mode else ast,  # Only pass None in fallback mode
                use_enhanced_scoring=True,  # Enable multi-scorer + context-aware scoring
                language=language,  # Pass language for ML scoring
                script_path=None  # Script path not available (content-based analysis)
            )
            ...
            # Calculate YaraContribution (AC #1)
            yara_contribution = None
            if yara_findings:
                # Use YaraScorer to calculate weighted scores
                from .scorers.yara import YaraScorer
                from .utils.config_loader import load_config
                try:
                    yara_config = load_config("config/patterns_config.yaml")
                except Exception:
                    yara_config = {}
                yara_scorer = YaraScorer(yara_config)
                yara_raw_score, _ = yara_scorer.score(yara_findings)

                # Get Yara weight from config (default 0.15)
                yara_weight = yara_config.get('scorer_weights', {}).get('yara', 0.15)
                yara_weighted = yara_raw_score * yara_weight

                # Calculate contribution percentage (rough estimate based on legacy scoring)
                # In legacy mode, we don't have a final score, so estimate contribution
                total_findings_score = len(all_findings) * 10  # rough estimate
                contribution_pct = (yara_weighted / total_findings_score * 100) if total_findings_score > 0 else 0
                contribution_pct = min(contribution_pct, 100)  # cap at 100%

                yara_contribution = YaraContribution(
                    enabled=self.yara_loaded,
                    matches=len(yara_findings),
                    score_contribution=f"{contribution_pct:.1f}%",
                    raw_score=yara_raw_score,
                    weighted_score=yara_weighted,
                    rules_matched=[f.pattern_id for f in yara_findings]
                )
                logger.debug(f"YaraContribution: {len(yara_findings)} matches, raw={yara_raw_score:.2f}")
            ...
            result = AnalysisResult(
                ...
                yara_contribution=yara_contribution,
                metadata={
                    # Script information
                    ...
                    'yara_duration': round(yara_duration, 3),
                    'aggregation_duration': round(aggregation_duration, 3),
                    ...
                    'yara_findings_count': len(yara_findings),
                    'llm_findings_count': len(llm_findings),
                    'pre_dedup_findings': pre_dedup_count,
                    ...
                    'paranoia_level': paranoia_level,
                    'obfuscation_detected': obfuscation_detected,
                    'yara_available': self.yara_loaded,
                    'yara_rules_loaded': self.yara_engine.load_stats.loaded if self.yara_engine and self.yara_engine.load_stats else 0,
                    'llm_available': llm_available,
                    'llm_error': llm_error,
                    ...
                    # Score breakdown for explain reporter (includes context-aware explanations)
