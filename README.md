...
## Getting Started
Each docker image is managed in its own directory. The directory should be named the same as the image name (without the organization prefix). We prefer using a dash (`-`) as a seperator in the name. All image directories are located under the `docker` dir.
