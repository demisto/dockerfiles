...
* **platform**: The platfroms to build the docker images on. The default is `linux/amd64,linux/arm64`. Change the build platform only if you are unable to build the image on linux/arm64, all images should be built on linux/amd64. For more information about multi-arch build, read the [official documentation](https://docs.docker.com/desktop/multi-arch/).
