...
function build_hardening_manifest {
  if registryone_login; then
    ...
    docker run -it $DOCKER_IMAGE /bin/sh -c "cd ~;pip uninstall -y -r /$REQUIREMENTS;pip cache purge;pip install -v --no-deps --no-cache-dir --log /tmp/pip.log -r /$REQUIREMENTS;cat /tmp/pip.log;exit" | grep Added >> $DOCKER_PACKAGES_METADATA_PATH
...
function commit_ironbank_image_to_repo_one {
  ...
  #git checkout -B $NEW_BRANCH_NAME origin/$NEW_BRANCH_NAME || git branch --set-upstream-to=origin/$NEW_BRANCH_NAME $NEW_BRANCH_NAME || git checkout -B $NEW_BRANCH_NAME
