...
WELCOME_MSG = 'Thank you for your contribution. Your generosity and caring are unrivaled! Rest assured - our Docker ' \
...
WELCOME_MSG_WITH_GFORM = 'Thank you for your contribution. Your generosity and caring are unrivaled! Make sure to ' \
                         ...
                         'so our Docker wizard @{selected_reviewer} will know the proposed changes are ready to be ' \
                         'reviewed.'
...
def main():
    ...
    dockerfiles_repo = gh.get_repo(f'{org_name}/{repo_name}')

    pr_number = payload.get('pull_request', {}).get('number')
    pr = dockerfiles_repo.get_pull(pr_number)
    ...
    if pr.base.ref == 'master':
        ...
        existant_branches = dockerfiles_repo.get_git_matching_refs(f'heads/{branch_prefix}')
        ...
        master_branch_commit_sha = dockerfiles_repo.get_branch('master').commit.sha
        # create new branch
        print(cyan(f'Creating new branch "{new_branch_name}"'))
        dockerfiles_repo.create_git_ref(f'refs/heads/{new_branch_name}', master_branch_commit_sha)
    ...
    # Add a security architect reviewer if the PR contains security dockerfiles items
